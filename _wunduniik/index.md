---
title: Wunduniik
permalink: /wunduniik/
search_exclude: true
nav_exclude: true
layout: skyground_readme
modlist_id: wunduniik
project_banner:
  image: /assets/Images/Skyrim/enb_2026-2-28_18-36-11_274.webp
  subtitle: "Flagship Skyrim SE list with visuals, gameplay overhauls, and deep content."
  stats:
    - label: Mods
      value: "2900+"
    - label: Size
      value: "200 GB"
    - label: Version
      value: "1.6.1170"
---

# Wunduniik

{% include components/project_banner.html %}

Wunduniik is a comprehensive modlist for The Elder Scrolls V: Skyrim Anniversary Edition, focusing on enhanced graphics, immersive gameplay, and extensive content additions.

## Quick Links

- [Installation Guide]({{ '/wunduniik/installation-guide/' | relative_url }})
- [Changelog]({{ '/wunduniik/changelog/' | relative_url }})
- [Known Issues]({{ '/wunduniik/known-issues/' | relative_url }})
- [Load Order]({{ '/wunduniik/loadorder/' | relative_url }})

## About This Modlist

This modlist transforms Skyrim into a visually stunning and deeply immersive experience. It includes hundreds of carefully selected and configured mods that work together seamlessly.

### Key Features

- **Enhanced Graphics**: High-quality textures, improved lighting, and stunning visual effects
- **Gameplay Overhauls**: Improved combat, magic systems, and character progression
- **Expanded Content**: New quests, locations, NPCs, and equipment
- **Quality of Life**: UI improvements, bug fixes, and convenience features

## Support

For help and support, join our [Discord server](https://discord.gg/vKuB7nazBk).
