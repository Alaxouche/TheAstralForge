---
layout: skyground_readme
title: Changelog
nav_order: 2
---

In this section you will find changelogs for the modlist since its release. You can track down precisely everything that gets added, removed, fixed, tweaked and updated.
<div style="margin-bottom: 1rem;"></div>

<details markdown="1">
<summary>v2.2.0.0 - April 28, 2026</summary>

**Update Importance:** Medium  
**NOT SAVE COMPATIBLE**

___

# Additions:
- Added Mercenaries and Mayhem - a GRiNDTerra Outpost Expansion
- Added Into the Void Bug Fix
- Added Declared Contraband
- Added BetterConsole - Free lanes compatibility
- Added Real Volii Atmosphere
- Added AA-105B Rifle
- Added AA-105B Free Lanes Patch
- Added StarGym - One More Rep

# Fixes and Tweaks:
- Fixed some remaining issues introduced by out of date mods
- Fixed shadows visuals
- Fixed some performance issues
- Rebalanced some weapons

# Updates:
- Updated Rabbit's Real Lights New Atlantis
- Updated Produce (Fruits and Vegetables) and Beer Crates
- Updated More Visualized Docking
- Updated TN's Power Tools - Cutter - Arc Welder - Auto-Rivet
- Updated POIs Hostiles Diversity
- Updated Legendary Module Recycler
- Updated Starfield Engine Fixes - SFSE
- Updated Seeking out Stores - Navigate Using City Info Kiosks
- Updated Immersive Wait and Sleep Menu - Enhanced and Fully Configurable
- Updated Less Intrusive Notifications
- Updated HUD Info Widget
- Updated NPCs Use Items
- Updated Astrogate
- Updated True Space
- Updated Atmospheric Audio
- Updated Dark Universe - Takeover
- Updated Deadly Effective - Minimal Skills Overhaul
- Updated Resupply - Functional Supplies and Decorations
- Updated Red Mile Terror
- Updated Lunokhod-1 Landmark
- Updated Starvival - Immersive Survival Addon
- Updated Dark Future Texture Pack
- Updated Suit Up - Realistic Armor
- Updated Starshake - Vizualized Recoil
- Updated Astral UI
- Updated Astral UI - Quicklist + Usearch plugin
- Updated POI Faction Diversity
- Updated Starborn Space Suit Visor FX Plus
- Updated Skurkbro's Retexture Project - Landscape
- Updated Allied Arms weapon pack

</details>

<details markdown="1">
<summary>v2.1.1.0 - April 20, 2026</summary>

**Update Importance:** Medium  
**NOT SAVE COMPATIBLE**

___

# Additions:
- Added HUD Info Widget
- Added HideHUD
- Added Lunokhod-1 Landmark
- Added Trackers Alliance Space Bounties Fix

# Fixes and Tweaks:
- Fixed crafting menu issues
- Fixed some HUD issues and implemented useful HUD informations
- Removed too OP weapons

# Removals:
- Removed Richer Merchants
- Removed Gemini Rifle
- Removed MIAO 06D by SealCT
- Removed More Dramatic Grav Jumps
- Removed Auto Hide HUD and Widgets - SFSE

# Updates:
- Updated More Dramatic Grav Jumps
- Updated NPCs Use Items
- Updated True Space
- Updated The Lodge Awakened
- Updated Where Did the Lights Go

</details>

<details markdown="1">
<summary>v2.1.0.0 - April 18, 2026</summary>

**Update Importance:** MAJOR  
**NOT SAVE COMPATIBLE**

___

# Additions:
- Added Old World 50 Cal Barrett M82 M95 (Standalone and Replacer)
- Added M4A1 - Assault Rifle
- Added G36C Redux
- Added Elysium 2154 - AKM
- Added All Vehicles Have Storage
- Added Weapon Sound Fixes
- Added Weapons Sound Replacer
- Added Royal Leveled Enemies - Shattered Space
- Added Red Mile Expanded
- Added Positively New Atlantis
- Added Milkdrinker's New Atlantis Mesa Trees Reborn
- Added Legendary Module Recycler
- Added Skill ReSpec Terminal
- Added HUD Info Widget
- Added HideHUD
- Added Lunokhod-1 Landmark
- Added Trackers Alliance Space Bounties Fix
- Added Unarmed sneak animation replacer
- Added POIs Hostiles Diversity
- Added Skurkbro's Retexture Project - 2K And Optimized
- Added TN's Power Tools - Cutter - Arc Welder - Auto-Rivet
- Added Odyssey - A True RPG (Collection)
- Added Richer Vendors - 10x merchant gold and 1 day restock
- Added Project Warfare - Balance Patch

# Fixes and Tweaks:
- Fixed major instability due to some not compatible mods
- Fixed Walter's missing legs
- Fixed invisible clutter and furniture
- Fixed missing weapon sounds
- Improved performance greatly 
- Provided a performance profile (ulta is on by default)
- improved cities for both performance and look (akila, atlantis, cydonia...)
- Rebalanced weapons and removed too OP ones

# Removals:
- Removed Undelayed Menus
- Removed Show Star Names
- Removed Less Spongy Enemies
- Removed Baka Quit Game Fix
- Removed Simple Offence Suppression SFSE
- Removed ADS FOV Removed (SFSE)
- Removed Realistic Stellar Nomenclature
- Removed Terrifying Terrormorphs
- Removed Always Play Landing Animations (APLA)
- Removed Suppressor Sound Overhaul
- Removed Clear anti-glare glass for outpost windows
- Removed Patches - More Terrormorph Robot Typo
- Removed NASA Items
- Removed Starfield Powerful Rifle Sounds Replacer
- Removed Slightly Better Map Icons
- Removed POI - Abandoned UC Radar Station
- Removed Richer Merchants
- Removed Gemini Rifle
- Removed MIAO 06D by SealCT
- Removed No hit confirmation sound
- Removed More Dramatic Grav Jumps
- Removed Immersive Cargo Halls
- Removed Auto Hide HUD and Widgets - SFSE
- Removed Immersive Renames
- Removed HBI Breakable Beer Crate
- Removed HBI Breakable Wine Glass Bottle
- Removed HBI Breakable Mug
- Removed HBI Breakable Foam Container
- Removed HBI Breakable Other Cups
- Removed HBI Breakable Vase
- Removed HBI Breakable Bowl And Plate
- Removed HBI Breakable Window in RLs Collection Loose File ver
- Remvoed First Person Messages - RTFP
- Removed Skurkbro's Retexture Project - Starfield - WIP
- Removed Auto Spacesuit Hider - Custom Conditionalizer - Vanilla Bug Fixes - SFSE
- Removed Increased Weapon and Combat Audio Bass with Enhanced Fidelity (Starfield)
- Removed Upscaling Fix 2 - FSR - DLSS - XeSS
- Removed more starborn Terrormorph Robot
- Removed New Atlantis Awaits
- Removed Save Game Thumbnails - SFSE
- Removed Scrap Metal Weapons AIO
- Removed Rabbit's Real Lights New Atlantis Awaits

# Updates:
- Updated Dodge Framework SFSE
- Updated Console Command Runner Updated
- Updated Integrated Ship Optimization Terminal
- Updated Roverhaul - Simple Vehicle Fuel and Management
- Updated More Dramatic Grav Jumps
- Updated StarPatcher
- Updated NPCs Use Items
- Updated True Space
- Updated Real Venus Atmosphere
- Updated NPCs Use Items
- Updated Real Titan Atmosphere
- Updated Mantle of the Mantis
- Updated The Lodge Awakened
- Updated Astrogate
- Updated Starborn Space Suit Visor FX Plus
- Updated Show Power Names
- Updated Where Did the Lights Go
- Updated Sit To Add Ship to Fleet
- Updated Ship Power Allocation Fix - Revert Unwanted Power Changes
- Updated Seeking out Stores - Navigate Using City Info Kiosks
- Updated Human Activity on Earth - Man-made Points of Interest
- Updated Scared of Shootings - NPCs react to shootings
- Updated Variable Contraband Prices - Contraband More Valuable in Locations with Scans
- Updated Summoning of Ship - Ship Remote Control - Land - Take off - Set gravity
- Updated Swiftly Order Squad - Multiple Followers - Group Commands
- Updated POI Variations - No More Duplicates
- Updated Following Asteroids Fix - Stalking Objects Solution
- Updated Security Checkpoints Dialog Fixes
- Updated Improved Ship Pathing

</details>

<details markdown="1">
<summary>v2.0.0.0 - April 16, 2026</summary>

**Update Importance:** MAJOR  
**SAVE COMPATIBLE**

___

# Additions:
- Added Mass Matters
- Added Missing Nasa Decal Fix
- Added True Space
- Added High-Resolution Planet Textures
- Added Astral UI
- Added PraxisUI Inventory
- Added Save Game Thumbnails - SFSE
- Added Dazra Uncut
- Added Starfield Revised - Player Speed
- Added HyperGuardian Mass Effect armor - Standalone
- Added More Tattoo Colors
- Added More Piercings
- Added More Hair Colors
- Added Real Titan Atmosphere
- Added Permanent POIs - Untouched Horizons
- Added Supply Mission Fix
- Added HBI Breakable Window in RLs Collection
- Added HBI Breakable Bowl And Plate
- Added HBI Breakable Other Cups
- Added HBI Breakable Foam Container
- Added HBI Breakable Vase
- Added HBI Breakable Wine Glass Bottle
- Added HBI Breakable Beer Crate
- Added First Person Messages - RTFP
- Added Immersive Renames
- Added Real Time Form Patcher (SFSE)
- Added Royal Patch Hub And Difficulty Configurators
- Added Royal Galaxy - A Starfield Revamp And Cut Content Restoration
- Added Royal Weapon Balance Fixes Vanilla Weapon And Combat Overhaul
- Added NAT Station Lake Windows
- Added Varuunkai Reborn- a GRiNDTerra Mod
- Added GRiNDTerra Small Tweaks and Patches
- Added Vanilla Biomes Enhanced- A GRiNDTerra Mod
- Added Allied Arms weapon pack
- Added Dataslate Dark
- Added Clear (Not Invisible) Glass and Reflections
- Added Hair Overhaul - New Colors
- Added RTM's Yellow Jump Seat Exposure Packs
- Added Keep Generic Crew Members Around
- Added Scanner Closest Path Highlight Improvements
- Added Save Game Thumbnails - SFSE
- Added Auto Hide HUD and Widgets - SFSE
- Added Avontech Suit Power Cores
- Added Transfer Spaceships Across Universes - SFSE
- Added Real O2
- Added Detailed Reference Info - SFSE
- Added Davefield - Loot Container UI
- Added Astrogate
- Added Prototype Grendel Skin
- Added Bigger Systems - Expanded Star Maps
- Added Zoom for Perfect Landing
- Added Temple Rings Main Menu Video 4K
- Added Real Venus Atmosphere
- Added TA - Dogfight First-person View Tweek
- Added Integrated Ship Optimization Terminal
- Added Realistic Venus Appearance
- Added No Build Below Bay or Invalid Docker Position or Misaligned Landing Gear Errors
- Added No Minimum Landing Gear
- Added No Engine Power Limits
- Added No Reactor Class Requirement
- Added No Cockpit Limit
- Added No Landing Bay or Docker Limits
- Added No Grav Drive Weight Limit
- Added LOC - Life of Crime (Player Bounty Overhaul and Criminal Missions)
- Added No Weapon Power or Count Limits
- Added No Reactor Limit
- Added Rabbit's Real Lights Asteroid Mansion
- Added VBB (Voluptuous Body for Beauties) - New Proportions for Female Characters

# Fixes and Tweaks:
- Reworked the list to work with Free Lanes update
- Fixed player speed
- Rebalanced weapons damages to be uniform
- Reinstalled some mods to be in loose files, to stay below the limit of 300 ba2
- Revampted the UI mods
- Provided optional (and initially disabled) nude option
- Fixed some instability and CTDs

# Removals:
- Removed Rudy HQ - More epic Milky Way texture
- Removed Material Icons for StarUI
- Removed Realistic Recoil
- Removed Ryujin Dataslate
- Removed StarValor
- Removed StarValor - Patch Hub
- Removed SECS - Ship Exterior Camera Separation
- Removed Praedy's SF Milky Way re-texture
- Removed Realistic Venus Atmosphere
- Removed Weapon Accuracy Overhaul

# Updates:
- Updated Starfield Script Extender (SFSE)
- Updated Baka Achievement Enabler (SFSE)
- Updated Baka Hold to Sprint
- Updated Address Library for SFSE Plugins
- Updated Quick Lowered Weapons - First Person
- Updated More NPCs Have Routines and Stores Have Schedules
- Updated Ship HUD Tweaks
- Updated Buildings and or People- A GRiNDTerra Mod
- Updated Compact Mission UI
- Updated UC Military Overhaul - Complete Edition
- Updated Compact Crew Menu UI
- Updated Akila City Rises
- Updated No Reactor Limit
- Updated No Engine Power Limits
- Updated No Weapon Power or Count Limits
- Updated Console Command Runner Updated
- Updated No Shield Limit
- Updated Mech Enemies
- Updated Trackers Alliance - Fight Or Flight
- Updated Scopes Without Overlays
- Updated Balanced Ballistics (And Lasers)
- Updated Real Fuel - Immersive Exploration
- Updated GRiNDTerra Planetary POI Generation
- Updated Resupply - Functional Supplies and Decorations
- Updated Starfield Engine Fixes - SFSE
- Updated Atmospheric Audio
- Updated Galactic Mesh Fixes
- Updated Starvival - Immersive Survival Addon
- Updated Functional Reflex Sights
- Updated Deadly Effective - Minimal Skills Overhaul
- Updated Titanfall 2 Weapon Pack
- Updated Peak Performance - Minimalist Needs
- Updated Lodecs Custom Armory (LCA)
- Updated Cassiopeia Papyrus Extender
- Updated Spacefaring Economy
- Updated Ship Vendor Framework (SVF)
- Updated See day length when landing
- Updated Take All to Ship - Quick Item Transfer - SFSE
- Updated Cargo Reconsidered
- Updated Player Eyes Stuck at the Dialogue Target Bug Fix - SFSE
- Updated Immersive Landing Ramps
- Updated Novafield Patch Hub
- Updated Accurate Aiming
- Updated Main and Pause Menu Tweaks
- Updated Immersive Driving - SFSE
- Updated A hiss means it's close. If you hear a pop... (Realistic Bullet Cracks by Greenpeacekiller Port)
- Updated Colony One - fully featured UC space colony
- Updated No Vehicle Mouse Steering - SFSE
- Updated Cleanfield - A No-Intro Videos And Clean Menu Fix
- Updated Real Gravitational Anomalies- A GRiNDTerra Mod
- Updated GRiNDTerra Planetary POI Generation
- Updated Better Standing Mirror with Reflection
- Updated Take Your Time 
- Updated Rug Seam Fix
- Updated Better LilyPads
- Updated Useful Brigs
- Updated No More Idiotic Landings
- Updated Avontech Suit Power Cores
- Updated Suit Up - Realistic Armor
- Updated Less Creepy Crowd NPCs - Reduced Staring and Headtracking
- Updated Cost Of Spacing
- Updated StarPatcher
- Updated No Forced Vehicle Aim Assist - SFSE
- Updated No Vehicle Auto Handbrake - SFSE
- Updated Real Fuel - Immersive Exploration
- Updated Affinity Reworked
- Updated Tell Me Your Name - SFSE
- Updated Roverhaul - Simple Vehicle Fuel and Management
- Updated Customizable Player Spacesuit Nameplate
- Updated Spaceships Redone - MonstrrMagic Texture Series
- Updated Auto Spacesuit Hider - Custom Conditionalizer - Vanilla Bug Fixes - SFSE
- Updated Starware Search Engine - SFSE
- Updated Slightly Better Map Icons
- Updated No Vehicle Weapon Aim Oxygen Penalty - SFSE
- Updated SKK Fast Start New Game (Starfield)
- Updated Ship Power Control Tweaks
- Updated Places Of Intrigue- GRiNDTerra POI
- Updated Furniture Redone - MonstrrMagic Texture Series
- Updated Unofficial Starfield Patch - USFP
- Updated Skurkbro's Retexture Project
- Updated CharGenMenu
- Updated Paradiso Evolved
- Updated HPE Better Rifle Fire Smoke Effects
- Updated HPE Better Pistol Fire Smoke
- Updated HPE Better Old Earth Gun Fire Smoke
- Updated Ads and Signs Redone - MonstrrMagic Texture Series
- Updated Rampart-17
- Updated ARX-15
- Updated Fauna- A GRiNDTerra Mod
- Updated The Dark Side of Neon - An Ebbside Overhaul
- Updated Random Creature and Robot Size- A GRiNDTerra Mod
- Updated The Lodge Awakened
- Updated SFSE ASI Loader Updated
- Updated Encounters- a GRiNDTerra Mod
- Updated Starfield Memory Fixes
- Updated Starfield High Priority Launcher
- Updated Desolation - POI Overhaul
- Updated Dynamic Inventory Models - SFSE
- Updated Better Explosives - An Explosive Overhaul
- Updated Artifacts Redone - MonstrrMagic Texture Series
- Updated NPC Sync Walk Speed

</details>

<details markdown="1">
<summary>v1.0.1.0 - March 31, 2026</summary>

**Update Importance:** SMALL  
**SAVE COMPATIBLE**

___

# Additions:
- Added Atmospheric Audio

# Updates:
- Updated Resupply - Functional Supplies and Decorations
- Updated Starfield Engine Fixes - SFSE
- Updated Galactic Mesh Fixes
- Updated Starvival - Immersive Survival Addon
- Updated Red Mile Terror

</details>

<details markdown="1">
<summary>v1.0.0 - Initial Release</summary>

**Update Importance:** Major  

---

### Additions:
- Initial release of Extrasolar Containment Protocol

</details>
